...
class Model(UserDict):
    ...
    def rotate(self, R, conjugate=False, transpose=False, infinitesimal=False):
        r"""
        Rotate momenta and real space coordinates with rotation matrix ``R``:

        .. math:: H(k, x, d) → H(±R k, R x, ±R d)

        with the transformation preformed for all vector-like quantities the
        model depends on.

        Momenta ``k`` are inverted by conjugate xor transpose,
        hopping_vector ``d`` is inverted by transpose.
        The matrix part of the model is not conjugated or transposed!

        If ``infinitesimal=True``, perform infinitesimal spatial rotation
        generated by ``exp(i λ R)``:

        .. math:: H(k) → sum_{ij} 1j * dH(k)/dk_i R_{ij} k_j

        plus the same transformation for ``x`` and ``d``.
        """
        if infinitesimal and (conjugate or transpose):
            raise ValueError("If `infinitesimal=True` `conjugate` and `transpose` "
                             "must be False.")
        for key, vec in self._special_symbols.items():
            if not (vec is None or R is None or len(vec) == R.shape[0]):
                raise ValueError("Shape of rotation matrix {} incompatible with "
                                 "number of {} variables {}.".format(R, key, vec))

        if infinitesimal:
            def trf(key):
                return sum([sympy.diff(key, var[i]) * R[i, j] * var[j]
                             for i, j in product(range(R.shape[0]), repeat=2)
                             for var in self._special_symbols.values() if var is not None
                           ])
            return 1j * self.transform_symbolic(trf)
            ...
            for key, vec in self._special_symbols.items():
                if vec is None:
                    continue
                if R is None:
                    vec_prime = sympy.Matrix(vec)
                else:
                    vec_prime = R @ sympy.Matrix(vec)
                # change sign as appropriate
                vec_prime = signs[key] * vec_prime
...
class BlochModel(Model):
    ...
    def rotate(self, R, conjugate=False, transpose=False):
        ...
        sign = (-1 if (conjugate^transpose) else 1)
        if R is not None:
            assert len(momenta) == R.shape[0], (momenta, R)
            # do rotation on hopping vectors with transpose matrix
            R_T = np.array(R).astype(float).T
            result.data = {BlochCoeff((sign * R_T) @ hop, coeff): copy(val) for (hop, coeff), val in self.items()}
        else:
            result.data = {BlochCoeff(sign * hop, coeff): copy(val) for (hop, coeff), val in self.items()}
        return result
