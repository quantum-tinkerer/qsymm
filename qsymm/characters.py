...
class LittleGroup(SpaceGroup):
    def __init__(self, generators, k=None, double_group=None, allow_inconsistent=False, _tests=False, tol=1e-9):
        ...
        super().__init__(self.generators, double_group=double_group, _tests=_tests, tol=tol)
        if (self.antiunitary_generator is not None and
            not abs(prop_to_id((self.antiunitary_generator**2).R)[1] - 1) < 1e-5):
    ...
    def _physical_irreps(self, irreps):
        # Find irreps compatible with antiunitary
        # This works for little group, but only if TR^2 propto 1.
...
# +
# from qsymm.groups import cubic, rotation, time_reversal
...
# +
# # %lprun -f PointGroup.regular_representation.func pg.irreps
