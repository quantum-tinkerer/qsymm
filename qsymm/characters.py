...
class PointGroup(set):
    ...
    def decompose_U_rep(self):
        ...
        if not self.consistent_U:
            raise ValueError('The U representations are inconsistent, run `fix_U_phases` first')
...
g = qsymm.groups.cubic(tr=False, ph=False, generators=False, spin=1/2, double_group=False)
pg = PointGroup(g)
