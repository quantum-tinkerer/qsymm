...
def test_check_symmetry():
            ...
            for _ in range(5):
                ham = sum(
                    [
                        ...
                        for member in Hamiltonian_family
    ...
    with raises(ValueError):
        check_symmetry(
            [Model({1: np.eye(2)}, momenta=["k_x"])], [chiral(1, np.diag([1, -1]))]
        )
...
def test_bloch_generator():
    ...
    sphi = sympy.pi
    rotR = sympy.Matrix(
        [[sympy.cos(sphi), -sympy.sin(sphi)], [sympy.sin(sphi), sympy.cos(sphi)]]
    )
    ...
    assert all(
        [
            np.allclose(value, value.conj())
            for member in pretty
    ...
    norbs = [(0, 2)]
    family = bloch_family(
        hopping_vectors, symmetries, norbs, onsites=False, bloch_model=True
    )
    assert len(family) == 6, "Incorrect number of members in the family"
    check_symmetry(family, symmetries)
    pretty = make_basis_pretty(family, num_digits=3)
...
def test_continuum_variables():
    ...
    assert all([term in result for term in terms]), (
        "Incorrect momentum terms returned by continuum_variables."
    )
    ...
    assert all([term in result for term in terms]), (
        "Incorrect limitations on powers in continuum_variables."
    )
    all_powers = [[1], [1, 3]]
    momenta = _commutative_momenta
    ...
    terms = continuum_variables(
        dim, total_power, all_powers=all_powers, momenta=momenta
    )
    assert all([term in result for term in terms]), (
        "New momentum variables treated incorrectly."
    )


def test_pairing_generator():
    mU = np.array([[0.0, 1.0j], [1.0j, 0.0]])
    mR = np.array([[-1]])
    ...
    for ph_square in ph_squares:
        for phase in phases:
            ...
            from_pairing = continuum_pairing(
                symmetries,
                dim,
                total_power,
                ph_square=ph_square,
...
def test_pretty_basis():
    ...
    assert len(pfam) == len(fam), (
        "Basis sparsification failed, returned a smaller basis."
    )
