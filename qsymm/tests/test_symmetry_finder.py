...
def productrep(n, gens):
    ...
    for _ in range(n - 1):
        genn = np.array(
            [
                (
                    np.kron(genn[i], np.eye(len(gens[i])))
                    + np.kron(np.eye(len(genn[i])), gens[i])
                )
...
def sumrep(*args):
    ...
    genn = np.array(
        [
            la.block_diag(*[args[j][i] for j in range(len(args))])
...
def test_cont_finder():
    ...
    assert solve_mat_eqn(H, H, hermitian=True, traceless=True).shape == (
        3,
        2 * n,
        2 * n,
    )
    assert solve_mat_eqn(H, H, hermitian=False, traceless=True).shape == (
        3,
        2 * n,
        2 * n,
    )
    assert solve_mat_eqn(H, H, hermitian=True, traceless=False).shape == (
        4,
        2 * n,
        2 * n,
    )
    ...
    Hs = [
        h1,
        np.array([la.block_diag(h1[i], h1[i]) for i in range(len(h1))]),
...
def test_disc_finder(verbose=False):
    ...
    for sym in sym_list:
        ...
        Hs = Model(
            {kwant_continuum.sympify("a_" + str(i)): H for i, H in enumerate(Hs)},
            momenta=[],
        )
        ...
        assert np.sum(np.abs([T, P, C])) + 1 == len(sg), print(
            [(g.conjugate, g.antisymmetry) for g in sg], T, P, C
        )
        if T:
    ...
    # with the same symmetry in a random basis, also tests _reduce_hamiltonian
    # The square of TR and PH may change, only test 4th power
    sym_list = "A", "AI", "AII", "AIII", "BDI", "CII", "D", "C", "CI"
    dim = 4  # Dimension of each block
    # skip DIII because it has symmetry with dim<=4
    for sym in sym_list:
    ...
    # time-reversal symmetry. Here, there are two ways to construct the
    ...
    if verbose:
        print("TR**2", TR.dot(TR.conj()))
    assert np.allclose(TR.dot(TR.conj()), np.eye(TR.shape[0])) or np.allclose(
        TR.dot(TR.conj()), -np.eye(TR.shape[0])
    )
    # Is a symmetry of all Hamiltonians
    for H in Hs:
        assert tr.apply(Hs) == Hs
    ...
    for _ in range(8):
        ...
        Hs.append(
            la.block_diag(
                h2,
                t_mat.dot(h2.conj()).dot(t_mat.T.conj()),
                h1,
                t_mat.dot(h1.conj()).dot(t_mat.T.conj()),
            )
        )
    Hs = Model(
        {kwant_continuum.sympify("a_" + str(i)): H for i, H in enumerate(Hs)},
    ...
    sg = {time_reversal(0), particle_hole(0), chiral(0)}
    sg, Ps = discrete_symmetries(Hs, sg)
    assert sorted([P.shape for P in Ps]) == sorted(
        [(1, 4 * dim, dim), (1, 4 * dim, dim), (2, 4 * dim, dim)]
    )
    assert len(sg) == 2
    tr = list(sg)[0]
    TR = tr.U
...
def test_continuum():
    ...
    C4 = PointGroupElement(
        np.array([[1, 0, 0], [0, 0, 1], [0, -1, 0]], int), False, False, None
    )
    C3 = PointGroupElement(
        np.array([[0, 0, 1], [1, 0, 0], [0, 1, 0]], int), False, False, None
    )
    ...
    # First model
    ham1 = (
        "hbar^2 / (2 * m) * (k_x**2 + k_y**2 + k_z**2) * eye(2) +"
...
def test_bloch():
    ...
    Mx = PointGroupElement(sympy.ImmutableMatrix([[-1, 0], [0, 1]]), False, False, None)
    ...
    # First example
    ham6 = (
        "m * (cos(k_x) + cos(1/2*k_x + sqrt(3)/2*k_y) + cos(-1/2*k_x + sqrt(3)/2*k_y))"
    )
...
def test_symmetries_edge_cases():
    ...
    lattices = [
        ...
        (
            [[1, 1 / 3, 0], [1, -1 / 3, 0], [0, 0, np.sqrt(10) / 3]],
            8,
            "base centered orthorhombic corner case",
        ),
        ([[1, 0, 0], [0, 2, 0], [1 / 10, 0, 4]], 4, "primitive monoclinic"),
        ([[1, 0, 0], [0, 1, 0], [0, 1 / 10, 2]], 4, "primitive monoclinic"),
        (
            [[1, 1 / 3, 0], [1, -1 / 3, 0], [1 / 10, 0, 4]],
            4,
        ...
        (
            [[1, 0, 5], [-1 / 2, np.sqrt(3) / 2, 5], [-1 / 2, -np.sqrt(3) / 2, 5]],
            12,
            "rhombohedral",
        ),
        ([[np.sqrt(3) / 2, 1 / 2, 0], [0, 1, 0], [0, 0, 2]], 24, "hexagonal"),
        (
            [[np.sqrt(3) / 2, 1 / 2, 0], [0, 1, 0], [0, 0, 1]],
            24,
            "hexagonal equal length corner case",
        ),
    ]
    for periods, n, name in lattices:
        group = bravais_point_group(periods, tr=False, ph=False)
