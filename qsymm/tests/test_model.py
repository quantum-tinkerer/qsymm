...
def test_dense_algebra():
    ...
    assert 0 + d == d
    assert d @ c == 3 * Model(
        {1: 2 * vec, "x": 6 * vec, "x**2": 3 * vec, "x**3": 9 * vec}
    )
    assert c.T() @ d == 3 * a * b
...
def test_dense_algebra_bloch():
    ...
    assert a * b == BlochModel(
        {1: 2, "e**(I*k_x)": 6, "e**(I*2*k_x)": 3, "e**(I*3*k_x)": 9}
    )
    assert (a - a) * b == {}
    assert (a - a) * (b - b) == {}
    # vector models
...
def test_sparse_algebra():
    ...
    assert (
        d + 2 * LinearOperator((3, 3), matvec=lambda v: mat @ v)
        == Model({1: 4, "x**2": 3}) * mat
    )
    assert d @ c == 3 * Model(
        {1: 2 * vec, "x": 6 * vec, "x**2": 3 * vec, "x**3": 9 * vec}
...
def test_to_bloch_coeff():
    ...
    assert allclose(bc[0], np.array([1.0 / 2, np.sqrt(3)]))
    assert sympy.simplify(bc[1]) == sympy.simplify(
        sympy.sqrt(5) * e ** (2 * c0 + I * c1)
    )
    ...
    key = (
        sympy.sqrt(5)
        * e ** (2 * c0)
        * e ** (I * (k_x / 2 + np.sqrt(3) * k_y + c1 * k_y))
    )
    with raises(ValueError):
        bc = _to_bloch_coeff(key, momenta)
...
def test_Model():
    ...
    keys = [
        _symbol_normalizer(e ** (I * k_x + 2 * I * k_y)),
...
def test_BlochModel():
    ...
    with raises(ValueError):
        bm = BlochModel(m)
    m = Model(
        {c1 * e ** (I * k_y): 3 * np.eye(2), np.sqrt(3) * e ** (I * k_x): np.eye(2)},
        momenta=["k_x", "k_y"],
        normalize=True,
    )
    bm = BlochModel(m)
...
def test_Model_subs():
    ...
    Ham = Model(
        {
            c0 * e ** (-I * (k_x / 2 + k_y) + sympy.sqrt(2)): T,
            c1 * e ** (I * (4 * k_x + 3 * k_y)): 2 * T,
        },
        momenta=["k_x", "k_y"],
    )
    ...
    right_Ham = Model(
        {
            c0 * e ** (-I * k_x / 2): T * np.exp(np.sqrt(2)) * np.exp(-1.5j),
    ...
    right_Ham = BlochModel(
        {
            BlochCoeff(np.array([-0.5, -1]), One()): T * 3,
